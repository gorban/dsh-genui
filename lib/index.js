...
//#endregion
//#region ../../opentiny/genui-sdk/packages/core/dist/index.js
const q = objectType({
...
const I = objectType({
	...
	params: arrayType(stringType()).optional().describe("函数参数名列表")
}).describe("JS 函数包装");
...
const A = ["禁止设置饼图的 `settings.radius`"];
function l(n) {
...
function apply(ctx, config) {
	const text = He("Vue", D, { customActions: [{
		name: "continueChat",
		...
		parameters: {
			...
			properties: { message: {
				type: "string",
				description: "User-facing follow-up text."
			} },
			required: ["message"]
		}
	}] });
